...
def create_plots(timestamp: str, data: List[Data], num_qubits: List[int]):
    ...
    x_vals = [
        ...
        if data_point.num_qubits_server == num_qubits[0]
    ]

    selfish_makespans = [
        [
            [
                data_point.makespan_selfish
                for data_point in d.data_points
                if data_point.num_qubits_server == q
            ]
            for q in num_qubits
        ]
        for d in data
    ]
    cooperative_makespans = [
        [
            [
                data_point.makespan_cooperative
                for data_point in d.data_points
                if data_point.num_qubits_server == q
            ]
            for q in num_qubits
    ...
    cooperative_succ_probs = [
        [
            [
                data_point.succ_prob_cooperative
                for data_point in d.data_points
                if data_point.num_qubits_server == q
            ]
            for q in num_qubits
    ...
    for d in range(0, len(data)):
        for i in range(0, len(num_qubits)):
            ...
            plt.plot(
                x_vals,
                cooperative_makespans[d][i],
                label=f"{data[d].meta.num_clients} clients, Coop, {q} server qbts",
                marker="s",
            )
            plt.plot(
                x_vals,
                selfish_makespans[d][i],
                label=f"{data[d].meta.num_clients} clients, Self, {q} server qbts",
                marker="o",
            )
    ...
    create_png(
        timestamp + "_" + xlabel + num_clients_str + num_qubits_str + "_makespan"
    )
    plt.cla()
    ...
    for d in range(0, len(data)):
        for i in range(0, len(num_qubits)):
            ...
            plt.plot(
                x_vals,
                cooperative_succ_probs[d][i],
                label=f"{data[d].meta.num_clients} clients, Coop, {q} server qbts",
                marker="s",
            )
            plt.plot(
                x_vals,
                selfish_succ_probs[d][i],
                label=f"{data[d].meta.num_clients} clients, Self, {q} server qbts",
                marker="o",
            )
    ...
    create_png(
        timestamp + "_" + xlabel + num_clients_str + num_qubits_str + "_succ_prob"
    )
    plt.cla()
...
if __name__ == "__main__":
    ...
    param_strings = [
        ...
        "host_instr_time",
        "host_peer_latency",
        "qnos_instr_proc_time",
        "t2",
    ]  # "single_gate_dur", "single_gate_fid",
