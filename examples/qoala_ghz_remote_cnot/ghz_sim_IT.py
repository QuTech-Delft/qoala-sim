...
# create_procnode_cfg
def perfect_trapped_ion_qdevice(
    id_node: any, name: str, num_qubits: int
) -> QDevice:
    ...
    node = Node(ID=id_node,name=name, qmemory=processor)
    return QDevice(node=node, topology=topology)

def setup_components_trapped_ion(
    id_node: any, name: str, num_qubits: int, latencies: QnosLatencies = QnosLatencies.all_zero()
) -> Tuple[QnosProcessor, UnitModule]:
    qdevice = perfect_trapped_ion_qdevice(id_node, name, num_qubits)
...
def trapNode (id_node: any, name: str, num_qubits: int) -> LhiProcNodeInfo:
    topology = LhiTopologyBuilder.trapped_ion_default_perfect_gates(num_qubits)
    latencies = LhiLatencies(
        host_instr_time=1000, qnos_instr_time=2000, host_peer_latency=3000
    )
    return LhiProcNodeInfo(name=name, id=id_node, topology=topology, latencies=latencies)
...
def run_sim(num_iterations: int, interactions: dict, programs: dict, node_ids: dict) -> AppResult:
    ...
    ntfs = [TrappedIonNtf(), TrappedIonNtf(), TrappedIonNtf()]
