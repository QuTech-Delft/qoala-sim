...
class NodeScheduler(Protocol):
    ...
    def run(self) -> Generator[EventExpression, None, None]:
        while True:
            ...
            if self._last_cpu_tasks_pids is not None:
                for pid in self._last_cpu_tasks_pids:
                    self._logger.info(f"Ack completion of CPU task {pid}")
                    self.schedule_next_for(pid)

            if additional_qpu_pids:
                for pid in additional_qpu_pids:
                    self._logger.info(f"Ack completion of lone QPU task {pid}")
                    self.schedule_next_for(pid)
...
class ProcessorScheduler(Protocol):
    def __init__(
        ...
        self._pending_pids_finished_tasks: Set[int] = set()
...
class EdfScheduler(ProcessorScheduler):
    ...
    def handle_task(self, task_id: int) -> Generator[EventExpression, None, None]:
        ...
        if success:
            ...
            self._pending_pids_finished_tasks.add(task.pid)
...
class CpuEdfScheduler(EdfScheduler):
    ...
    def is_message_available(self, tid: int) -> bool:
        ...
        self._task_logger.debug(f"I currently have messages for these tasks: {messages} from {remote_name}")
...
class QpuEdfScheduler(EdfScheduler):
    ...
    def are_resources_available(self, tid: int) -> bool:
        ...
        if isinstance(task, SinglePairTask):
            ...
            virt_ids_to_allocate = [virt_id for virt_id in virt_ids if self._memmgr.phys_id_for(task.pid, virt_id) is None]
    ...
    def update_status(self) -> None:
        ...
        self._task_logger.info(f"Tasks with no predecessors: {epr_no_predecessors}")
        ...
        self._task_logger.info(f"Tasks blocked on resources: {blocked_on_resources}")
    ...
    def run(self) -> Generator[EventExpression, None, None]:
        while True:
            ...
            if Status.EPR_GEN in self.status.status:
                self._task_logger.debug("hello EPR_GEN")
                task_id = self.status.params["task_id"]
                yield from self.handle_task(task_id)
            elif Status.NEXT_TASK in self.status.status:
                self._task_logger.debug("hello NEXT_TASK")
                task_id = self.status.params["task_id"]
                yield from self.handle_task(task_id)
