...
def _qpu_sched_for_resource_check(
    alloc_block: Optional[str],
    with_full_task_graph: bool = True,
    check_qubit_ancestry: bool = True,
):
...
def test_resources_available_ancestry_check_disabled():
    # The check is opt-in. With it off (the default) an already-allocated
    # qubit is accepted regardless of which block allocated it, preserving
    # the behaviour of programs that do not annotate every dependency.
    sched, tid = _qpu_sched_for_resource_check(
        alloc_block="b_unrelated", check_qubit_ancestry=False
    )
    assert sched.are_resources_available(tid)
...
if __name__ == "__main__":
    ...
    test_resources_available_ancestry_check_disabled()
